...
def parse_tool_reply(reply: str, shortlist: list[str]) -> str:
    ...
    for name in shortlist:                      # shortlist order = model's order
        token = name.lower()
        if token in lowered.split() or token in lowered.replace("`", " ").split():
            return name
    for name in shortlist:                      # last resort: substring
