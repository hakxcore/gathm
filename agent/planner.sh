...
# --- Plan Templates ---
# Pre-defined plan templates for common composite tasks
declare -A PLAN_TEMPLATES

PLAN_TEMPLATES["security_audit"]='[
...
PLAN_TEMPLATES["encrypt_and_transfer"]='[
...
PLAN_TEMPLATES["ip_investigation"]='[
...
# --- Task Decomposition ---
# Analyze a natural language task description and create an execution plan
cmd_plan() {
    ...
    if [[ -n "$matched_template" ]]; then
        ...
        # Generate a dynamic plan based on keyword analysis
...
_display_plan() {
    ...
    local plan="${PLAN_TEMPLATES[$template_key]}"
...
# Generate a dynamic plan from keyword analysis
_generate_dynamic_plan() {
    ...
    # Keyword-to-tool mapping for dynamic plans
    local -A TASK_TOOLS=(
        ["weather"]="weather"
        ["forecast"]="weather"
        ["ip"]="geo"
    ...
    for keyword in "${!TASK_TOOLS[@]}"; do
        if echo "$task_lower" | grep -q "$keyword"; then
            local tool="${TASK_TOOLS[$keyword]}"
            # Avoid duplicate tools
            local already_seen=false
            for seen in "${seen_tools[@]}"; do
                if [[ "$seen" == "$tool" ]]; then
                    already_seen=true
                    break
                fi
            ...
            if ! $already_seen; then
                ...
                steps+=("{\"step\":$step,\"tool\":\"$tool\",\"args\":\"\",\"description\":\"$desc\"}")
                seen_tools+=("$tool")
            fi
        fi
    done
...
cmd_execute_plan() {
    ...
    echo "$plan" | grep -oP '\{[^}]+\}' | while IFS= read -r step_json; do
        step=$((step + 1))
